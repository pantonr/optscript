...
def fetch_daily_metrics(analytics, days=7):
    ...
    try:
        ...
        date_range_info = [f"Last 7 Days ({start_date} to {end_date})"]
...
def write_to_seven_day_view(sheets, data):
    """Write data to a tab named '7-Day View'"""
    try:
        sheet = sheets.open_by_key(SPREADSHEET_ID)
        
        # Check if '7-Day View' tab exists, if not create it
        try:
            worksheet = sheet.worksheet('7-Day View')
        except gspread.exceptions.WorksheetNotFound:
            worksheet = sheet.add_worksheet(title='7-Day View', rows=50, cols=15)
        ...
        try:
            ...
            header_row = 3
            worksheet.format(f"A{header_row}:J{header_row}", {
            ...
            total_col = 9  # Column I (assuming data spans columns A-J)
            worksheet.format(f"I4:I{4+metrics_count}", {
                "backgroundColor": {"red": 0.95, "green": 0.95, "blue": 0.8},
                "textFormat": {"bold": True}
            })
            
            # Format the Change column - Fixed 'true' to 'True'
            worksheet.format(f"J4:J{4+metrics_count}", {
            ...
            # This will format cells green for positive changes and red for negative
            worksheet.conditional_format(f"B{4+metrics_count+1}:I{4+metrics_count+metrics_count}", {
                "type": "TEXT_CONTAINS",
                "values": [["↑"]],
        ...
        print(f"Successfully updated '7-Day View' tab with dashboard data.")
...
def main():
    print("Starting GA4 dashboard data collection process...")
    analytics, sheets = authenticate()
    print("Authentication successful")
    
    print("Fetching website metrics for the last 7 days...")
    dashboard_data = fetch_daily_metrics(analytics, days=7)
    
    if dashboard_data:
        print("Writing data to 7-Day View tab...")
        success = write_to_seven_day_view(sheets, dashboard_data)
        if success:
            print("Process completed successfully! Dashboard data written to '7-Day View' tab")
    else:
